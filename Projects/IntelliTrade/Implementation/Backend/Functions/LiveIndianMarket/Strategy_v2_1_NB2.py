...
def saveTradeStatusToDisk():
    ...
    with open(os.path.join(directory_path, 'strategy_v2_1_NB2_TradeState.json'), 'w+') as output_file:
...
def loadTradeStatusFromDisk():
    ...
    # 
    directory_path = parameter_dict['directory_path']
    with open(os.path.join(directory_path, 'strategy_v2_1_NB2_TradeState.json'), 'r') as input_file:
...
def activeThreadsMonitoring():
    """
    """
    file_path = os.path.join(GOOGLEDRIVE_TRADE_LOGS_DIR, 'LIVE', 'ActiveThreadMonitoring', 'ActiveThreadsName_strategy_v2_1_NB2.txt')
...
def initializeMainProcess():
    ...
    file_name = 'strategy_v2_1_NB2'+'_'+datetime.now().strftime('%Y%m%d_%H%M%S')+'.log'
    logger = initializeLogger(directory_path=directory_path, file_name=file_name)
    # 
    parameter_dict = {
        'strategy_name'          : 'strategy_v2_1_NB2',
        'instrument_name'        : 'NIFTYBANK',
        'step_size'              : 100,
        'sell_trade_time'        : '0915',
        'buy_trade_time'         : '1524',
        'expiry_week_ahead'      : 0,
        'lower_strike_bandwidth' : 800,
        'upper_strike_bandwidth' : 800,
        'lower_margin_bandwidth' : 200,
        'upper_margin_bandwidth' : 200,
        'lower_margin_threshold' : 1200,
        'upper_margin_threshold' : 1200,
...
strategy_v2_1_NB2_MainThread_exitFlag = False
def strategy_v2_1_NB2_MainThread():
    """
    """
    global strategy_v2_1_NB2_MainThread_exitFlag
    ...
    # 
    while not strategy_v2_1_NB2_MainThread_exitFlag:
        strategy_v2_1_NB2_Main()
    ...
    logger.info('Exiting -> strategy_v2_1_NB2_MainThread')
...
def shutdownAllThreads():
    ...
    global raiseNetworkErrorThread_exitFlag
    # 
    strategy_v2_1_NB2_MainThread_exitFlag = True
...
strategy_v2_1_NB2_MainThread_exitFlag = False
strategy_v2_1_NB2_MainThread_thread = threading.Thread(target=strategy_v2_1_NB2_MainThread, name='strategy_v2_1_NB2_MainThread', daemon=True)
strategy_v2_1_NB2_MainThread_thread.start()
