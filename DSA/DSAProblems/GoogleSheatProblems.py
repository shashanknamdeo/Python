...
def maxArea(array):
    ...
    """
    len_array = len(array)
    current_max_area = min(array[0], array[1])
    # 
    for i in range(0, len_array-1):
        for j in range(len_array-1, i, -1):
            if i != 0:
                if (j-i) < current_max_area/array[i] and i != 0:
                    break
            elif current_max_area < min(array[i], array[j])*(j-i):
                current_max_area = min(array[i], array[j])*(j-i)
    # 
